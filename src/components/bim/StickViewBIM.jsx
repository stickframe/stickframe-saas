...
import StickQuoteBIMModal from "./StickQuoteBIMModal";
import {
  listarBimElementos,
  upsertBimElemento,
...
export default function StickViewBIM({ obraId, user, onAddToOrcamento, modelos = [] }) {
  ...
  const [showBIMQuote, setShowBIMQuote] = useState(false);
    ...
    <>
      ...
      <div style={{border:"1px solid var(--line)",borderRadius:16,overflow:"hidden",background:"#0f0e12"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"rgba(10,9,14,.97)",borderBottom:"1px solid rgba(255,255,255,.05)"}}>
          ...
          {ifcFile && (
            <button onClick={() => setShowBIMQuote(true)} style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(152,25,21,.85)",border:"1px solid rgba(152,25,21,.6)",borderRadius:7,padding:"5px 13px",fontFamily:"inherit",fontSize:11.5,fontWeight:700,color:"#fff",cursor:"pointer"}}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" style={{width:12,height:12}}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
              StickQuote™ BIM
            </button>
          )}

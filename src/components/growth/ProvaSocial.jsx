/**
 * ProvaSocial — seção de prova social da landing (StickFrame Conversion Layer™ C.7).
 * Importado do Claude Design ("landing/Prova Social.html") e integrado:
 *  - React global → imports ESM;
 *  - CTA aponta para /calcular e dispara cta_clicked (GA4);
 *  - métricas viram props (default = números reais do banco) para wiring futuro.
 */
import { useState, useRef, useEffect } from "react";
import { analytics } from "../../utils/analytics";

const ps_C = {
  brick: '#981915', brickDk: '#7d1411', ink: '#232225', ink2: '#57514a',
  muted: '#8c847a', bgLight: '#f7f4f0', border: '#e7e1d8', sage: '#4f7d57', steel: '#3b6ea5',
};

// ── count-up gated on intersection ──
...
export default function ProvaSocial({ m2 = 12500, simulacoes = 20, orcamentos = 18 }) {
  return (
    <section style={ps_S.section} id="prova-social">
      <style>{ps_CSS}</style>
      <div style={ps_S.wrap}>
        ...
        <div style={ps_S.metrics} className="ps-metrics">
          <Metric value={m2} unit="m²" label="simulados na calculadora pública" />
          <div style={ps_S.metricDiv} className="ps-mdiv" />
          <Metric value={simulacoes} prefix="+" label="simulações de obra" />
          <div style={ps_S.metricDiv} className="ps-mdiv" />
          <Metric value={orcamentos} label="orçamentos técnicos gerados" />
        </div>
        ...
        <div style={ps_S.ctaWrap}>
          <a href="/calcular" style={ps_S.cta} className="ps-cta"
             onClick={() => analytics.ctaClicked("Calcular minha obra", "prova-social")}>
            Calcular minha obra

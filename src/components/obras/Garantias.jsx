  ...
  return (
    <div>
      ...
      {loading ? <p>Carregando...</p> : garantias.length === 0 ? (
        <div style={{ textAlign: "center", padding: 40, color: "var(--text-muted)" }}>🛡️ Nenhuma garantia cadastrada.</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {garantias.map(g => (
            <div key={g.id} style={{ background: "var(--bg-card)", borderRadius: 10, padding: 16, borderLeft: `4px solid ${COR[g.status]}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 14 }}>{g.item}</div>
                {g.fornecedor && <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Fornecedor: {g.fornecedor}</div>}
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <button onClick={() => { setForm({...g}); setModal(g); }}
                  style={{ fontSize: 11, padding: "3px 8px", background: "none", border: "1px solid var(--border)", borderRadius: 4, cursor: "pointer" }}>✏️</button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div style={{ background: "var(--bg-card)", borderRadius: 12, padding: 24, width: "min(460px,95vw)" }}>
            ...
            {[["Item *","item","text"],["Fornecedor","fornecedor","text"],["Data início","data_inicio","date"],["Data fim *","data_fim","date"]].map(([l,k,t]) => (
              <label key={k} style={{ display: "block", marginBottom: 12 }}>
                ...
                <input type={t} value={form[k]||""} onChange={set(k)} style={{ display: "block", width: "100%", marginTop: 3, padding: 8, borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-input)", boxSizing: "border-box" }} />
              </label>
            ))}
            <label style={{ display: "block", marginBottom: 16 }}>
              <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Observações</span>
              <textarea value={form.observacoes||""} onChange={set("observacoes")} rows={2} style={{ display: "block", width: "100%", marginTop: 3, padding: 8, borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-input)", resize: "vertical", boxSizing: "border-box" }} />
            </label>

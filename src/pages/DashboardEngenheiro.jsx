      ...
      {obrasAtivas.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {obrasAtivas.map((obra) => {
            ...
            const fasePct = faseIdx >= 0 ? ((faseIdx + 1) / FASES.length) * 100 : 0;

            return (
              <div key={obra.id} style={{
                background: "#fff", borderRadius: 14,
                border: `1px solid ${C.border}`,
                borderLeft: `4px solid ${prazoAtrasado ? C.danger : C.red}`,
                padding: "20px 24px",
              }}>
                {/* Título */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{
                      ...
                      color: Number(obra.progresso) >= 80 ? C.success : C.text,
                    }}>
                      {obra.progresso || 0}%
                    </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: obra.prazo_fim ? 14 : 0 }}>
                  ...
                  <div style={{
                    ...
                    border: desvioPos ? `1px solid ${C.danger}44` : `1px solid ${C.border}`,
                  }}>
                    ...
                    {previsto > 0 && (
                      <div style={{ marginTop: 8 }}>
                        <BarraProgresso valor={despTotal} max={previsto} cor={desvioPos ? C.danger : C.success} />
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ background: C.darker, borderRadius: 8, padding: "12px 14px", border: `1px solid ${C.border}` }}>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 800, color: medObra.length > 0 ? C.warning : C.success }}>
                      {medObra.length}
                    </div>
                    <div style={{ fontSize: 10, color: C.muted, marginTop: 3 }}>
                      {fmt(medObra.reduce((a, m) => a + (m.valor || 0), 0))} a faturar
                    </div>
                    <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>
                      receita: {fmt(recTotal)}
                    </div>
                  </div>
                  ...
                  <div style={{ background: C.darker, borderRadius: 8, padding: "12px 14px", border: `1px solid ${aptObra.length > 0 ? C.danger + "44" : C.border}` }}>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 800, color: aptObra.length > 0 ? C.danger : C.success }}>
                      {aptObra.length}
                    </div>
                ...
                {prazoFim && (
                  <div style={{
                    ...
                    <span style={{ fontSize: 12, color: C.muted }}>
                      Entrega prevista: <strong>{prazoFim.toLocaleDateString("pt-BR")}</strong>
                    </span>
                    <span style={{
                      fontSize: 12, fontWeight: 800,
                      color: prazoAtrasado ? C.danger : diasRestantes <= 14 ? C.warning : C.success,
                    }}>
                      {prazoAtrasado
